//https://github.com/isaacg1/colors
...
pub fn generate(size: u32, frontier_groups: usize, debug_frequency: Option<usize>) -> ImageBuffer<Rgb<u8>, Vec<u8>> {
	assert!(size <= 16);
	...
	for (i, color) in colors.into_iter().enumerate() {
		...
		let (location, frontier_index) = if i >= random_locs as usize {
			...
			let location = loop {
				let list = &locations_to_regions.iter().collect::<Vec<_>>();
				let &(&location, region) = list
