use regex::Regex;
use rocket::{serde::json::Json, Route, response::{status, content}};
use sea_orm::*;
use sea_orm_rocket::{Connection};

use crate::{models::*, pools::Db, viewmodels, admin_auth::AdminAuth};
...
#[get("/api/users")]
async fn get_users(conn: Connection<'_, Db>, _auth: AdminAuth) -> Json<Vec<viewmodels::ViewableUserData>> {
	...
	let users = users.into_iter()
		.map(|u| {
			let user::Model { id, email, password: _, is_admin } = u;
			viewmodels::ViewableUserData { id, email, is_admin }
		})
		.collect();
	Json(users)
}
#[post("/api/users/<old_id>", data = "<new_user_data>")]
async fn update_user(conn: Connection<'_, Db>, _auth: AdminAuth, old_id: i32, new_user_data: Json<viewmodels::UserData>)
	-> Result<status::Accepted<()>, status::BadRequest<content::RawJson<&'static str>>>
	{
	fn new_error(json: &'static str) -> Result<status::Accepted<()>, status::BadRequest<content::RawJson<&'static str>>> {
	...
	if new_user_data.id != old_id && user::Entity::find_by_id(new_user_data.id).one(db).await.unwrap().is_some() {
	...
	if new_user_data.email != user.email && user::Entity::find().filter(user::Column::Email.eq(new_user_data.email.clone())).one(db).await.unwrap().is_some() {
